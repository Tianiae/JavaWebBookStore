...
public class IndexAction extends BaseAction {
	...
	// 公共方法 提供公共查询数据
	private void front() {
		this.getRequest().setAttribute("title", "网上书城系统");
		List<Cate> cateList = this.cateService.getAllCate();
		this.getRequest().setAttribute("cateList", cateList);
		List<Book> hotList = this.bookService.getBookByHot();
	...
	public String index() {
		...
		for (Cate cate : cateList) {
			List<Book> bookList = this.bookService.getBookByCate(cate.getCateid());
			cate.setBookList(bookList);
			frontList.add(cate);
		}
	...
	public String addcart() {
		...
		String userid = (String) this.getSession().getAttribute("userid");
		Cart cart = new Cart();
		cart.setBookid(getRequest().getParameter("goodsid"));
	...
	public String checkout() {
		...
		if (cartList.size() == 0) {
			...
			for (Cart cart : cartList) {
				...
				details.setBookid(cart.getBookid());
				details.setNum(cart.getNum());
				details.setOrdercode(ordercode);
				details.setPrice(cart.getPrice());
				details.setReceivername(this.getRequest().getParameter("receivername"));
				details.setReceiverphone(this.getRequest().getParameter("receiverphone"));
				details.setAddress(this.getRequest().getParameter("address"));
				this.detailsService.insertDetails(details);
				Book goods = this.bookService.getBookById(cart.getBookid());
				goods.setSellnum("" + (Integer.parseInt(goods.getSellnum()) + Integer.parseInt(cart.getNum())));
				this.bookService.updateBook(goods);
	...
	public String cate(String id, String number) {
		...
		List<Book> tempList = this.bookService.getBookByCond(goods);
		int pageNumber = tempList.size();
		int maxPage = pageNumber;
		if (maxPage % 12 == 0) {
			maxPage = maxPage / 12;
	...
	public String all(String number) {
		this.front();
		List<Book> bookList = new ArrayList<Book>();
		List<Book> tempList = this.bookService.getAllBook();
		int pageNumber = tempList.size();
		int maxPage = pageNumber;
		if (maxPage % 12 == 0) {
			maxPage = maxPage / 12;
	...
	public String query(String name) {
		...
		List<Book> bookList = this.bookService.getBookByLike(goods);
		this.getRequest().setAttribute("bookList", bookList);
		return "users/list";
	}
	...
	public String detail(String id) {
		this.front();
		Book goods = this.bookService.getBookById(id);
